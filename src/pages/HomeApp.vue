...
<template>
  <AppShell v-if="ready" :projects="projects" mode="home" @home="() => {}" @add-project="addProject" @select-project="openProject" @global-settings="openSettings">
    <div class="projects-page"><div class="projects-header"><div><h1>Projects</h1><p>{{ projects.length ? `${projects.length} local ${projects.length === 1 ? 'repository' : 'repositories'}` : 'Add a local Git repository to browse files and review changes.' }}</p></div><button class="button-primary" @click="addProject"><i class="bi bi-plus" aria-hidden="true"></i> Add project</button></div>
      <section v-if="projects.length" class="project-list-panel" aria-label="Local projects"><button v-for="project in projects" :key="project.id" type="button" class="project-list-row" @click="openProject(project.id)"><span class="project-list-icon"><i class="bi bi-folder2" aria-hidden="true"></i></span><strong class="project-list-name">{{ project.name }}</strong><i class="bi bi-chevron-right" aria-hidden="true"></i></button></section>
      <div v-else class="projects-empty"><i class="bi bi-folder2-open" aria-hidden="true"></i><h2>No projects yet</h2><p>Choose a local Git repository to get started.</p><button class="button-outline" @click="addProject">Choose folder</button></div>
    </div>
