...
async function requestInStore<T>(storeName: 'projects' | 'reviews', mode: IDBTransactionMode, action: (store: IDBObjectStore) => IDBRequest<T>): Promise<T> {
...
export async function listReviews(projectId: string): Promise<ReviewRecord[]> {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = () => {
      ...
      for (const review of request.result.sort((a, b) => b.createdAt - a.createdAt)) {
        const key = JSON.stringify([review.branch, review.baseRef]);
        if (seen.has(key)) store.delete(review.id);
        else { seen.add(key); reviews.push(review); }
      }
    };
